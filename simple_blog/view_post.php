...
// ------------------------------------
// 2. LOGJIKA E FSHIRJES SË KOMENTIT (VETËM ADMIN) 🗑️
// ------------------------------------
if (isset($_GET['delete_comment_id'])) {
    if (!$is_admin) {
        ...
        // Përdorim prepared statements për siguri shtesë, por ruajmë formatin bazë
    ...
    if (!isset($_SESSION['username'])) {
        ...
        if (!empty($comment_text)) {
            ...
            if (mysqli_query($conn, $insert_sql)) {
                // Ridrejto veten për të shmangur dërgimin e dyfishtë (Post/Redirect/Get)
                header("Location: view_post.php?id=$post_id");
                exit();
            } else {
...
<head>
    ...
    <style>
        /* CSS-i i përfshirë në kod për stilizim */
...
<div class="container">
    <header>
        ...
        <small>Publikuar më: <?php echo $post['created_at'] ?? 'Data Mungon'; ?> (Autor: **<?php echo htmlspecialchars($post['author'] ?? 'Anonim'); ?>**)</small>
    ...
    <div class="full-post">
        ...
        <p><?php echo nl2br(htmlspecialchars($post['content'] ?? 'Përmbajtja Mungon')); ?></p>
    </div>
                ...
                <div class="comment">
                    <div class="comment-meta">
                        ...
                        <small>
                            ...
                            <?php 
                            // Opcion: Lejo fshirjen nga autori i komentit gjithashtu (Nëse ka user_id në sesion dhe DB)
                            // Nëse nuk keni user_id, përdorni vetëm kontrollin Admin:
                            if ($is_admin): 
                            ?>
                                <a 
